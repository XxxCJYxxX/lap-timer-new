...
**计时与记录**
- `performance.now()` 毫秒级精度计时，整数取整防浮点泄漏
- 实时 Delta：计时中对比 PB 累计分段，红绿实时显示快慢
- GPS 自动分段：接近中间航点时自动记录 split time（阈值 15m）
- 赛段级 PB 对比：每个分段显示与 PB 对应赛段的差值（绿快红慢）
- 三色成绩判定：首圈紫色 (PB) / 比上次快绿色 / 比上次慢黄色
- 赛后统计：总圈数、最快圈、平均圈速、稳定性（标准差）
- 手动分段按钮：与自动分段可混用
...
- 温度、湿度、风速、天气类型写入每条成绩记录详情

**导航**
- 地名搜索 + OSRM 驾车路线规划（免费，无需 API Key）
- 地图蓝色虚线路线 + 红色目的地标记
- 路线信息卡：距离、预计用时
- 一键「立刻开始」：自动创建赛道路线并切换到计时模式
...
- **v1.0** — 当前版本：多航点路线、自动分段、实时 Delta、赛后统计、起跑灯序、GPS 自动启停、目的地导航、天气记录、路线分享、防熄屏
